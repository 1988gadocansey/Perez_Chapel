...
if (isset($_POST[submit])) {
    ...
    if(empty($id)){
    $query=$sql->Prepare("INSERT INTO perez_demographics (NAME) VALUES('$name')");
        if($sql->Execute($query)){
             //logging
                    //$dpat=$help->getDepartmentName($department);
                    $event="Creation";
                    $activity="$_SESSION[USERNAME] has added $name demographics";
            ...
            header('location:viewDemographics.php');
        }
        else{
            header('location:addDemographics.php?error=1');
        }
     }
     else{
          $query=$sql->Prepare("UPDATE perez_demographics SET NAME='$name' WHERE ID='$id'");
        if($sql->Execute($query)){
             //logging
                    $dpat=$help->getDepartmentName($department);
                    $event="Creation";
                    $activity="$_SESSION[USERNAME] has updated $name demography";
                    $hashkey = $_SERVER['HTTP_HOST'];
                    $remoteip = $_SERVER['REMOTE_ADDR'];
                    $useragent = $_SERVER['HTTP_USER_AGENT'];
                    $mac = $login->getMac();
                                    ...
                                    <div>
                                        <?php
                                            ...
                                            if(isset($_GET[id])){
                                                ...
                                                $query=$sql->Prepare("SELECT * FROM perez_demographics WHERE ID ='$demography'");
                                        ...
                                        <form action="" method="post" class="form-horizontal row-border"   id="form" novalidate="" name="applicationForm"  v-form>
                                            ...
                                            <div class="form-group">
                                                <span id="item">
                                                    <label class="col-sm-3 control-label">Demography Name</label>
                                                    <div class="col-sm-6">
                                                        <input type="text" class="form-control" required="required" name="name" value="<?php echo $row->NAME;?>" v-model="name"  v-form-ctrl>
