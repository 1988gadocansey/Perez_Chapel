...
function get_money($value, $simple=false) {
	...
	if($value >= 0) {
		if($simple) {
			return '<span class="'.$class.'">$<strong>'.number_format($value, 2).'</strong></span>';
		} else {
			return '<span class="'.$class.'">$<strong>'.number_format($value, 2).'</strong> <small>'.m('value').'</small></span>';
		}
	} else {
		if($simple) {
			return '<span class="'.$class.'">-$<strong>'.number_format((-1)*$value, 2).'</strong></span>';
		} else {
			return '<span class="'.$class.'">-$<strong>'.number_format((-1)*$value, 2).'</strong> <small>'.m('value').'</small></span>';
		}
	}
}
